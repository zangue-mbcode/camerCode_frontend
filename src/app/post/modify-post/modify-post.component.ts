...
export class ModifyPostComponent implements OnInit {
  ...
  errorMessage: string = '';
  constructor(private postService: PostService, private auth: AuthService, private router: Router, private route: ActivatedRoute) {
      ...
      (params: Params) => {
        this.postService.getPostById(params.id).then(
          (post: any) => {
            ...
            this.htmlContent = this.post.htmlContent
          }
        );
      }
    );
  ...
  onSubmit() {
    ...
    this.postService.modifyPost(this.post._id ,post).then(
      (response: any) => {
